...
abstract class GraphStageLogic private[stream] (val inCount: Int, val outCount: Int) {
  ...
  final def failStage(ex: Throwable): Unit = {
    def stageName: String = try {
      val outerField = getClass.getDeclaredField("$outer")
      outerField.setAccessible(true)
      outerField.get(this) match {
        case MapAsyncUnordered(_, func) ⇒ s"MapAsyncUnordered(${func.getClass})"
        case Map(func)                  ⇒ s"Map(${func.getClass})"
        case Collect(func)              ⇒ s"Collect(${func.getClass})"
        case _                          ⇒ getClass.toString
      }
    } catch {
      case x ⇒ getClass.toString
    }

    val newEx = new RuntimeException(s"Failing stage in $stageName", ex).fillInStackTrace()
    var i = 0
    while (i < portToConn.length) {
      if (i < inCount)
        interpreter.cancel(portToConn(i))
      else
        interpreter.fail(portToConn(i), newEx)
      i += 1
    }
    setKeepGoing(false)
  }
